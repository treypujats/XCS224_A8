...
def precompute_rotary_emb(dim, max_positions):
    ...
    #Take floor of half the number of dimensions. 
    half_dim = dim // 2
    # theta_i = 1/10000^(2(i-1)/dim) for i in [1, dim/2]
    inv_freq = 1.0 / (10000 ** (torch.arange(0, half_dim, 1).float() / half_dim))
    positions = torch.arange(max_positions, dtype=torch.float).unsqueeze(1)  # (max_positions, 1)
    angles = positions * inv_freq.unsqueeze(0)  # (max_positions, half_dim)
...
def apply_rotary_emb(x, rope_cache):
    ...
    #Below uses B, nh and other parameters, but this is more clear to me. 
    batch_size, num_heads, seq_len, head_dim = x.shape

    # use rope_cache to get sin and cos
    ...
    # create rotations tensor 
    rot = torch.stack([cos, sin], dim=-1)  
    rot_complex = torch.view_as_complex(rot)  
    
    # apply the rotation
    rotated_complex = x_complex * rot_complex  
    
    # map it back to real numbers. 
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.rope:
            ...
            rope_cache = precompute_rotary_emb(head_dim, max_seq_len)
