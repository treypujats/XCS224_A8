...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # declare max_trunc legnth based on context size above
        max_trunc_length = self.max_context_size
        
        # create function to truncate doc no less than 4 and no greater than max_trunc length . Ensure we pad short lines first.
        if len(doc) < 4:
            doc = doc + ' ' * (4 - len(doc))
        trunc_len = random.randint(4, min(len(doc), max_trunc_length))
        #truncate
        doc = doc[:trunc_len]

        # Handle if doc is less than 3 lines
        if len(doc) < 3:
            prefix, masked_content, suffix = "", doc, ""
        else:
            
            # randomly split and set mask for prefix masked and suffix
            mask_len = random.randint(1, max(1, trunc_len // 2))
            mask_start = random.randint(0, trunc_len - mask_len)
            
            #create 3 strings, split on start and total length of the string
            prefix = doc[:mask_start]
            masked_content = doc[mask_start:mask_start + mask_len]
            suffix = doc[mask_start + mask_len:]

        # create masked string noted above. 
        masked_string = (
            prefix
            + self.MASK_CHAR
            + suffix
        ...
        masked_string += self.PAD_CHAR * max(0, num_pads)

        # create input and output in step 4. 
        input_str = masked_string[:-1]
        output_str = masked_string[1:]

        # create x and y tensors similar to above. 
        x = torch.tensor([self.stoi.get(c, 0) for c in input_str], dtype=torch.long)
        y = torch.tensor([self.stoi.get(c, 0) for c in output_str], dtype=torch.long)
